...
public class InspectionController {
    ...
    public List<OpticalPowerInspection> getResults(
        ...
        if (roundId != null) {
            if (neId != null && !neId.isEmpty()) {
                return inspectionService.getResultsByRound(roundId).stream()
                        .filter(r -> neId.equals(r.getNeId()))
                        .toList();
            }
            return inspectionService.getResultsByRound(roundId);
        }
        if (neId != null && !neId.isEmpty()) {
            return inspectionService.getResultsByNe(neId);
        }
        return inspectionService.getLatestResults();
    }
    ...
    public void exportExcel(@RequestParam(required = false) Long roundId,
        ...
        if (roundId != null) {
            data = inspectionService.getResultsByRound(roundId);
        } else {
            data = inspectionService.getLatestResults();
        }

        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setHeader("Content-Disposition", "attachment; filename=inspection_result.xlsx");
        ...
        try (Workbook workbook = new XSSFWorkbook();
            ...
            String[] columns = {"网元名称", "网元ID", "槽位", "端口", "支持光功率",
                    "光模块速率", "距离档", "模块型号", "波长",
            ...
            for (OpticalPowerInspection r : data) {
                ...
                row.createCell(2).setCellValue(r.getSlotNo() != null ? r.getSlotNo() : 0);
                row.createCell(3).setCellValue(r.getPortNo() != null ? r.getPortNo() : 0);
                row.createCell(4).setCellValue(Boolean.TRUE.equals(r.getSupported()) ? "是" : "否");
                row.createCell(5).setCellValue(r.getLaserType() != null ? r.getLaserType() : "--");
                row.createCell(6).setCellValue(r.getLaserDistance() != null ? r.getLaserDistance() : "--");
                row.createCell(7).setCellValue(r.getPartNumber() != null ? r.getPartNumber() : "--");
                row.createCell(8).setCellValue(r.getLaserWave() != null ? r.getLaserWave() : "--");

                // 功率值
                Cell txCell = row.createCell(9);
                Cell rxCell = row.createCell(10);
                ...
                // 状态标色
                Cell txStatusCell = row.createCell(11);
                ...
                Cell rxStatusCell = row.createCell(12);
                ...
                row.createCell(13).setCellValue(r.getLowThreshold() != null ? r.getLowThreshold() : 0);
                row.createCell(14).setCellValue(r.getHighThreshold() != null ? r.getHighThreshold() : 0);
                row.createCell(15).setCellValue(r.getInspectionTime() != null ? r.getInspectionTime().format(DT_FMT) : "");
                row.createCell(16).setCellValue(r.getFailReason() != null ? r.getFailReason() : "");
            }
